...
def run_query(query):
    ...
    response = loop.run_until_complete(generate_response(query))
    loop.close()
    return response
...
if st.button('Send'):
    input_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # Capturing the current time

    if user_input:
        try:
            answer = run_query(user_input)
            response_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # Capturing the current time
            st.text_area("Response:", value=f"{answer}\n\nMessage sent at: {input_time}\nResponse received at: {response_time}", height=200)
